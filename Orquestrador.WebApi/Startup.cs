        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddSwaggerGen(c =>
            {
                c.SwaggerDoc("v1", new Info
                {
                    Title = "Orquestrador",
                    Version = "v1",
                    Description = "Um microserviço para a gestão de Orquestrador.",
                    Contact = new Contact
                    {
                        Name = "Arquitetura",
                ...
                var xmlWebApiFile = Path.Combine(AppContext.BaseDirectory, $"Orquestrador.WebApi.xml");
                if (File.Exists(xmlWebApiFile))
                {
                    c.IncludeXmlComments(xmlWebApiFile);
                }

                var xmlApplicationFile = Path.Combine(AppContext.BaseDirectory, "Orquestrador.Application.xml");
