using Loja.Infrastructure.DI;
using Loja.WebApi.DI;
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddSwaggerGen(c =>
            {
                c.SwaggerDoc("v1", new Info
                {
                    Title = "LOJA",
                    Version = "v1",
                    Description = "Um microserviço para a gestão de Loja.",
                    Contact = new Contact
                    {
                        Name = "Arquitetura",
			...
			//	c.OperationFilter<RequiredHeaderParameters>();
				...
				//var xmlWebApiFile = Path.Combine(AppContext.BaseDirectory, $"Loja.WebApi.xml");
    //            if (File.Exists(xmlWebApiFile))
    //            {
    //                c.IncludeXmlComments(xmlWebApiFile);
    //            }

    //            var xmlApplicationFile = Path.Combine(AppContext.BaseDirectory, "Loja.Application.xml");
